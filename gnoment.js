...
import { now, getLocalTimeZone, CalendarDateTime, parseDate, parseDateTime, parseAbsolute, toCalendarDateTime, toZoned, startOfMonth, endOfMonth, startOfYear, endOfYear, ZonedDateTime } from '@internationalized/date';
...
const parseFlexibleDate = (input, timeZone = "UTC") => {
    if (input.length === 10) {
        ...
        return d;
    } else if (input.includes("Z") || input.includes("+") || input.includes("-")) {
...
class Gnoment {
    ...
    toString() {
        return this.format('ddd MMM DD YYYY HH:mm:ss [GMT]ZZ',this.zonedDateTime.timeZone);
    }
    ...
    format = (format, timezone) => {
        ...
        if (!timezone) {
          ...
          timezone = tzNamePart ? tzNamePart.value : '';
          // timezone = this.zonedDateTime.timeZone;
        }      
        ...
        const formatTokens = {
          ...
          // Hour
          'HH': d.toLocaleString('en-US', { ...timeOptions, hour: '2-digit', hour12: false }).replace(/24/g, '00'),
          'H': d.toLocaleString('en-US', { ...timeOptions, hour: 'numeric', hour12: false }).replace(/24/g, '0'),
          'hh': d.toLocaleString('en-US', { ...timeOptions, hour: '2-digit', hour12: true }).replace(/ AM| PM/g, ''),
          'h': d.toLocaleString('en-US', { ...timeOptions, hour: 'numeric', hour12: true }).replace(/ AM| PM/g, ''),
          
          // Minute
          'mm': d.toLocaleString('en-US', { ...timeOptions, minute: '2-digit' }).padStart(2, '0'),
          'm': d.toLocaleString('en-US', { ...timeOptions, minute: 'numeric' }),
          
          // Second
          'ss': d.toLocaleString('en-US', { ...timeOptions, second: '2-digit' }).padStart(2, '0'),
          's': d.toLocaleString('en-US', { ...timeOptions, second: 'numeric' }),
          
          // Millisecond
          'SSS': d.toLocaleString('en-US', { ...timeOptions, fractionalSecondDigits: 3 }).split('.')[1] || '000',
          
          // AM/PM
          'A': d.toLocaleString('en-US', { ...timeOptions, hour12: true, hour: 'numeric' }).split(' ')[1],
          'a': d.toLocaleString('en-US', { ...timeOptions, hour12: true, hour: 'numeric' }).split(' ')[1].toLowerCase(),
          
          // Timezone
          'ZZ': getTimezoneOffset(d, timezone).replace(':', ''),
          'Z': getTimezoneOffset(d, timezone),
          'z': timezone || Intl.DateTimeFormat().resolvedOptions().timeZone
...
export { gnoment, gnomentToCalendarDate, gnomentToCalendarDateTime, gnomentToZonedDateTime };
